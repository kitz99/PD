...
let rec reduce = function
  ...
  | (Op(Int (n1,_),Plus,Float (n2,_),loc),s) -> Some (Float (float_of_int(n1)+.n2,loc),s)   
  ...
  | (Op(Int (n1,_),Mul,Float (n2,_),loc),s) -> Some (Float (float_of_int(n1)*.n2,loc),s)  
  | (Op(Float (n1,_),Mul,Int (n2,_),loc),s) -> Some (Float (n1*.float_of_int(n2),loc),s)  
  | (Op(Float (n1,_),Mul,Float (n2,_),loc),s) -> Some (Float (n1*.n2,loc),s)   
  | (Op(Int (n1,_),Div,Int (n2,_),loc),s) when n2 <> 0 
    -> Some (Int (n1/n2,loc),s)             (*Op+*)
  | (Op(Int (n1,_),Div,Float (n2,_),loc),s) when n2 <> 0.0 
    -> Some (Float (float_of_int(n1)/.n2,loc),s) 
  ...
  | (Op(Float (n1,_),Mic,Int (n2,_),loc),s) -> Some (Bool (n1<=float_of_int(n2),loc),s)  
  | (Op(Int (n1,_),Mic,Int (n2,_),loc),s) -> Some (Bool (n1<=n2,loc),s)

  | (Op(Int (n1,_),Egal,Int (n2,_),loc),s) -> Some (Bool (n1=n2,loc),s)            (*Op<=*)
  | (Op(Float (n1,_),Egal,Float (n2,_),loc),s) -> Some (Bool (n1=n2,loc),s)
  ...
  | (Atrib(l, Float (n,_),loc),s) ->                                         (*Atrib*)
      Some (Skip loc, update (l,Float(n,"")) s) 
  | (Atrib(l, Int (n,_),loc),s) ->                                         (*Atrib*)
      Some (Skip loc, update (l,Int(n,"")) s)
